    ...
    for (i = 1; i < usersData.length; i += 2) {
      if (
        usersData[i].fullName
          .toLocaleLowerCase()
          .match(regex.toLocaleLowerCase()) != null
      ) {
    ...
    return (
      <>
        ...
        <div className="row profileCard">
          <div className="profileCardContent">
            {usersData[index].profilePicture === "" ? (
              <img
                src="https://bootdey.com/img/Content/avatar/avatar7.png"
                style={{ height: 100, width: 100 }}
                alt="User-Profile"
              ...
              <img
                src={usersData[index].profilePicture}
                style={{ height: 100, width: 100 }}
                alt="User-Profile"
              />
            )}
          ...
          <div className="profileCardContent">
            <h1></h1>
            <h6>Roles:</h6>
            {usersData[index].roles.map((role, i) => (
              <li key={i}>{role}</li>
            ))}
          </div>
          <div className="catagories">
            <div className="profileCardContent">
              <h6>Skills:</h6>
              {usersData[index].skills.map((skill, i) => (
                <li key={i}>{skill}</li>
              ))}
            </div>
            <div className="profileCardContent">
              <h6>To Improve:</h6>
              {usersData[index].toImprove.map((toImprove, i) => (
                <li key={i}>{toImprove}</li>
              ))}
            </div>
          </div>
    ...
    <>
      ...
      <div className="filter">
        ...
        <div className="filterOption">
          ...
          <select
            id="exForcesFilter"
            selected={filterExForces}
            onChange={(e) => setFilterExforces(e.target.value)}
          >
            <option value="" selected>
              Select:
            </option>
